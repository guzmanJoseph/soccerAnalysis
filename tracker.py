...
class Tracker:
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            # Change the goalkeeper to a player
            for object_ind, class_id in enumerate(detection_supervison.class_id):
                if cls_names[class_id] == "goalkeeper":
                    detection_supervison.class_id[object_ind] = cls_names_inverse["player"]

            # Track objects
            detections_with_tracks = self.tracker.update_with_detections(detection_supervison)

            # loops over tracks to check for players and referees
            ...
            # loops over tracks to check for ball (there is only one ball)
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        y2 = int(bbox[3])
        x_center, y = get_center_of_bbox(bbox)
    ...
    def draw_annotations(self, video_frames, tracks, team_ball_control):
        ...
        print(f"Number of track frames: {len(tracks['players'])}")
